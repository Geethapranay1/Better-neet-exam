...
const random = (bits) => {
    ...
    if (bits === 128)
        return (get() << 64n) | get();
    if (bits === 160)
        return (random(128) << 32n) | random(32);
    if (bits === 192)
        return (random(128) << 64n) | random(64);
    if (bits === 224)
        return (random(160) << 64n) | random(64);
    if (bits === 256)
        return (random(128) << 128n) | random(128);
    if (bits === 384)
        return (random(256) << 128n) | random(128);
    if (bits === 512)
        return (random(256) << 256n) | random(256);
    if (bits === 1024)
        return (random(512) << 512n) | random(512);
    if (bits === 2048)
        return (random(1024) << 1024n) | random(1024);
    /* GENERAL CASE */
    let index = 0;
    let result = 0n;
    while (index < bits) {
